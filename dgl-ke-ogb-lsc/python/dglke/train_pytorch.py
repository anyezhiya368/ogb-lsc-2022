...
def load_model_from_checkpoint(args, n_entities, n_relations, ckpt_path, ent_feat_dim, rel_feat_dim):
    ...
    print("model created, checkpoint to load")
...
def train(args, model, train_sampler, valid_samplers=None, test_samplers=None, rank=0, rel_parts=None, cross_rels=None, barrier=None, client=None):
    ...
    backward_time = 0
    best_valid_mrr = 0
    valid_candidate = np.load("/raid/wgh/WikiKG90Mv2/wikikg90m-v2/processed/val_t_candidate.npy")
    for step in range(args.start_epoch, args.max_step):
        ...
        if args.valid and (step + 1) % args.eval_interval == 0 and step > 1 and valid_samplers is not None:
            ...
            if rank == 0:
                ...
                log = valid_result_dict['h,r->t']['t_pred_top10']
                t_pred_top10 = np.ones_like(log) * (-1)
                for ii in range(log.shape[0]):
                    t_pred_top10[ii] = valid_candidate[ii][log[ii]]
                valid_result_dict['h,r->t']['t_pred_top10'] = t_pred_top10
                ...
                for procpos in range(args.num_proc):
                    os.remove(os.path.join(args.save_path, "valid_{}_{}.pkl".format(procpos, step)))
