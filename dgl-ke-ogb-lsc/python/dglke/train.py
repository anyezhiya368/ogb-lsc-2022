...
class ArgParser(CommonArgParser):
    def __init__(self):
        ...
        self.add_argument('--test_mode', default='test-dev', type=str,
                          help='Evaluate the model on the validation set in the training.')
        self.add_argument('--ckpt_path', default='', type=str)
        self.add_argument('--start_epoch', default=0, type=int)
...
def main():
    ...
    if len(args.ckpt_path) == 0:
        ...
        model = load_model_from_checkpoint(args, dataset.n_entities, dataset.n_relations, 
                       args.ckpt_path, dataset.entity_feat.shape[1], dataset.relation_feat.shape[1])
