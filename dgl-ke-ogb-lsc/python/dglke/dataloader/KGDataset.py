...
class KGDatasetWiki(KGDataset):
    ...
    def __init__(self, args, path, name='wikikg90m', test_mode='test-dev'):
        self.name = name
        ...
        self.valid_dict['h,r->t']['t_candidate'] = np.load(args.valid_candidate_path)
        self.test_dict = self.dataset.test_dict(test_mode)
        self.test_dict['h,r->t']['t_candidate'] = np.load(args.test_candidate_path)
...
def get_dataset(args, data_path, data_name, format_str, delimiter='\t', files=None, has_edge_importance=False, test_mode='test-challenge'):
    if format_str == 'built_in':
        if data_name == 'Freebase':
            ...
            dataset = KGDatasetWiki(args, data_path, test_mode=test_mode)
        else:
